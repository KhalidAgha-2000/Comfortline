import { IonContent, IonLabel, IonSelect, IonSelectOption, IonImg, IonPage, IonButton, IonCard, IonTitle, IonCardTitle, IonCardSubtitle, IonFooter, IonAccordionGroup, IonAccordion, IonItem, IonList, useIonActionSheet, IonActionSheet } from '@ionic/react'
import { useState } from 'react'
import FeedbackHeader from '../components/FeedbackHeader'
import Forms from '../components/Forms'
import CheckYellow from "../pages/CheckYellow.jpg";
const Feedback: React.FC = (props) => {
    ...
    // Continue and submit 
    const continueAndSubmit = () => {
        if (formSwitch > 5 || formSwitch == 5) {
            present({
    ...
    return (
        <IonPage>
            <IonContent fullscreen >
                <div className='bg-[#F5F5F5] min-h-screen'>
                    ...
                    {formSwitch == 1 ?
                        ...
                        : roomForm != '' && formSwitch == 2 ?
                            <Forms continueAndSubmit={continueAndSubmit} setFormSwitch={setFormSwitch} formSwitch={formSwitch} title='Thermal comfort' question='How do you find thermal comfort?' />

                            : formSwitch == 3 ?
                                <Forms continueAndSubmit={continueAndSubmit} setFormSwitch={setFormSwitch} formSwitch={formSwitch} title='AIR QUALITY' question='How do you find Air quality?' />

                                : formSwitch == 4 ?
                                    <Forms continueAndSubmit={continueAndSubmit} setFormSwitch={setFormSwitch} formSwitch={formSwitch} title='VISUAL comfort' question='How do you find visual comfort? (light level) ' />

                                    : formSwitch == 5 ?
                                        <Forms continueAndSubmit={continueAndSubmit} setFormSwitch={setFormSwitch} formSwitch={formSwitch} title=' ACOUSTIC COMFORT' question='How do you find acoustic comfort ? ' />

                                        : formSwitch == 1}

                    <IonFooter className="ion-no-border bg-[#F5F5F5] pt-24 mb-1 flex items-center justify-center">
                ...
                <IonActionSheet
                isOpen={formSwitch > 5 || formSwitch == 5}
                // onDidDismiss={() => setFormSwitch(1)}
                // cssClass="customAction"
                // header=''
                // // htmlAttributes={[<IonImg className='w-full bg-[red]' src="/assets/images/CheckYellow.jpg" alt="check" />]}
                // subHeader={"Continue"}
